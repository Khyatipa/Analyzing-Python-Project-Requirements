...
def main():
    ...
    with open(output_filename, 'w', newline='') as csvfile:
        ...
        csv_writer.writerow(
            ["Project Path", "Existing Package Count", "Generated Package Count", "Unique to package list",
             "Unique to requirements", "Common packages"])
        ...
        for project in os.listdir(project_directory):
            ...
            if os.path.exists(package_list_path) and os.path.exists(requirements_path):
                ...
                csv_writer.writerow([project_path, len(package_list), len(requirements), len(unique_to_package_list),
                                     len(unique_to_requirements), len(common_packages)])
